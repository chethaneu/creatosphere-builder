      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <Card 
              key={index}
              className="card-chrome hover:scale-105 hover:-translate-y-2 transition-all duration-500 animate-slide-up group relative overflow-hidden"
              style={{ animationDelay: `${index * 0.15}s` }}
            >
              {/* Hover gradient effect */}
              <div className="absolute inset-0 bg-gradient-to-br from-foreground/0 to-foreground/0 group-hover:from-foreground/5 group-hover:to-foreground/5 transition-all duration-500"></div>
              
              <CardHeader className="relative">
                <div className={`w-20 h-20 rounded-2xl bg-metallic flex items-center justify-center mb-6 text-primary-foreground group-hover:scale-110 transition-transform duration-500 relative shadow-lg`}>
                  <service.icon className="w-10 h-10 relative z-10" />
                </div>
                <CardTitle className="text-2xl mb-3 text-foreground group-hover:text-metallic transition-all duration-300">
                  {service.title}
                </CardTitle>
              </CardHeader>
              <CardContent className="relative">
                ...
                <div className="absolute bottom-0 left-0 w-0 h-1 bg-metallic group-hover:w-full transition-all duration-500"></div>
              </CardContent>
            </Card>
          ))}
        </div>
